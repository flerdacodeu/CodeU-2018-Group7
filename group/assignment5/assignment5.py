...
def find_first_different_letter(word1,word2):
    #returns position in words of first different letter, -1 is words are same (or atleast same until the end of the shorter word)
    length_of_word1 = len(word1)
    length_of_word2 = len(word2)
    i = 0
    while i < length_of_word1 and i < length_of_word2:
        if word1[i] != word2[i]:
            return i
        i += 1
    return -1
    ...
    while i < qlen:
        ...
        for child in children:
            ...
            if lettersGraph[child].get_number_of_parents() == 0:
                ...
                qlen += 1
        i += 1
    #if dictionary is consistent we will go through all lettersGraph, so if we haven't done that dictionary is inconsistent
    if qlen < len(lettersGraph):
        raise BaseException("The given dictionary is inconsistent")
    return alphabet

def find_alphabet(dictionary):
    """function that finds alphabet of given dictionary.It will return just one of the valid dictionaries.
