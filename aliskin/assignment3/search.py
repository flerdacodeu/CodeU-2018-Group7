...
def get_valid_neighbours(position, n_rows, n_cols):
    ...
    for i in range(row - 1, row + 2):
        for j in range(col - 1, col + 2):
            if (i == row) and (j == col):
                continue
            if is_valid_index(i, n_rows) and is_valid_index(j, n_cols):
                neighbours.append((i, j))
    return neighbours

def word_search_with_prefix(prefix, grid, dictionary, visited, queue):
    '''
    ...
    while len(queue) > 0:
        ...
        to_visit = deque(get_valid_neighbours((row, col), len(grid),
            len(grid[0])))
        words = words.union(word_search_with_prefix(prefix_new, grid, dictionary,
            visited_new, to_visit))
    return words
...
def word_search(grid, dictionary):
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            prefix = ''
            queue = deque([(row, col)])
            words = words.union(word_search_with_prefix(prefix, grid, dictionary,
                visited, queue))
